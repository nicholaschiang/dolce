...
/**
 * The collection "scores" are calculate based on the number of positive and negative
...
/**
 * Calculate the score percentage (out of 100%) to collection to users.
 */
...
export function getScoresSchema(scores: Serialize<Scores>): AggregateRating {
  return {
    ...
    'description':
      ...
      'critic reviews that are positive for a given runway collection. A Critic ' +
      'Score is calculated for a runway collection after it receives at least five ' +
      'reviews.',
    'name': 'Critic Score',
